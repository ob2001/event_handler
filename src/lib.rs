...
mod tests {
    ...
    fn empty_initialization() {
        let eh: EventHandler<TestEvents> = EventHandler::new(None, None, None);
        let e_v: Vec<TestEvents> = Vec::new();
        let em_v: Vec<Box<dyn Emitter<TestEvents>>> = Vec::new();
        let li_v: Vec<Box<dyn Listener<TestEvents>>> = Vec::new();
    ...
    fn stack_manipulation() {
        let mut eh: EventHandler<TestEvents> = EventHandler::new(None, None, None);
    ...
    fn listener_creation_and_addition() {
        let mut eh: EventHandler<TestEvents> = EventHandler::new(None, None, None);
        let e_v: Vec<Box<dyn Listener<TestEvents>>> = Vec::new();
        let li = TestListener { triggers: vec![TestEvents::E2] };

        assert_eq!(eh.get_listeners().len(), 0);
        assert_eq!(eh.get_listeners().type_id(), e_v.type_id());

        eh.register_listener(Box::new(li));

        assert_eq!(eh.get_listeners().len(), 1);
        assert!(eh.get_listeners()[0].triggers().contains(&TestEvents::E2));

        let lis: Vec<Box<dyn Listener<TestEvents>>> = vec![Box::new(TestListener {triggers: vec![TestEvents::E3]})];
